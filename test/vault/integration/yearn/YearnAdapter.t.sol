...
contract YearnAdapterTest is AbstractAdapterTest {
    ...
    function test__usdt_deposit_bob() public {
        overrideSetup(abi.encode(0xdAC17F958D2ee523a2206206994597C13D831ec7));

        deal(address(asset), address(bob), 1e6);

        vm.startPrank(bob);
        IERC20(asset).safeIncreaseAllowance(address(adapter), 1e6);

        adapter.deposit(1e6, address(bob));
        vm.stopPrank();
        emit log_uint(IERC20(asset).balanceOf(address(adapter)));
    }

    function test__usdt_deposit_this() public {
        overrideSetup(abi.encode(0xdAC17F958D2ee523a2206206994597C13D831ec7));
        ...
        adapter.deposit(1e6, address(this));
        emit log_uint(IERC20(asset).balanceOf(address(adapter)));
    }
